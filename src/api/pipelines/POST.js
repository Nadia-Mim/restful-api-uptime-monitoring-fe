...
export const createPipelineTemplate = async (payload) => {
    ...
    try {
        const res = await axios.post(`${Server.baseApi}/pipelines`, { ...payload, createdBy: authData?.userId }, {
            headers: { 'Content-Type': 'application/json', token: authData?.token }
        });
        return [true, res?.data?.data];
    } catch (err) {
        return [false, err?.response?.data?.error || 'Failed to create pipeline template'];
    }
};
